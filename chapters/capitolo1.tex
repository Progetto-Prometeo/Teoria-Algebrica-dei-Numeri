...
\begin{itemize}
	...
	\item Vale l'identità di Bezout? Ovvero, esistono $x=x_1+x_2\omega$ ed $y=y_1+y_2\omega$ tali che $x_i,y_i\in\mathbb{Z}$ e $3x+()y=1$?\\ Se esistessero tali $x$ ed $y$ avrei
	\begin{equation*}
	\begin{cases}
	3x_1+4y_1-5y_2=1\\
	3x_2+y_1+4y_2=0
...
\begin{esercizio}[Non unicità di fattorizzazione]
	...
	\begin{align*}
	\text{per $a=2$} \ \ \ \ \ \ \ \ \ \ &9=(2+\omega)(2-\omega)=(3)(3)\\
	\text{per $a=3$} \ \ \ \ \ \ \ \ \ \ &14=(3+\omega)(3-\omega)=(2)(7)\\
	\text{per $a=4$} \ \ \ \ \ \ \ \ \ \ &21=(4+\omega)(4-\omega)=(3)(7)
	\end{align*}
\end{esercizio}
...
\label{lezione6}
\subsection{La struttura delle classi di resto}
...
\begin{osservazione}[Costruzione di un anello di classe di resto]\
	...
	\end{equation*}
	ad $n$ fissato. Ovviamente è una relazione di equivalenza, basta sfruttare le solite proprietà dei divisori. \\Quindi $a$ e $b$ sono in relazione se hanno lo stesso resto nella divisione euclidea per $n$. Allora costruisco l'insieme delle classi di equivalenza
	\begin{equation*}
	\mathbb{Z}_n \simeq \sfrac{\mathbb{Z}}{\sim}
	\end{equation*}
	\begin{itemize}
		\item Siccome $\mathbb{Z}$ è un PID i suoi ideali sono principali, e sappiamo che hanno forma $(n)=n\mathbb{Z}$,
		\begin{equation*}
		\mathbb{Z}_n\simeq\sfrac{\mathbb{Z}}{(n)}\simeq\sfrac{\mathbb{Z}}{n\mathbb{Z}}
		\end{equation*}
		\item Si vede che $a+(n)=b+(n)\iff (a-b)\in(n)$
	\end{itemize}
\end{osservazione}
...
\begin{proof}\
	\begin{itemize}
		\item[(ben definita)] Chiaramente se $[a]_n=[b]_n$ allora $n$ divide $a-b$, ed allora anche $m$ li divide; quindi sono nella stessa classe anche rispetto ad $m$.
		\item[(omomorfismo)] Ovvio per struttura delle classi.
		\item[(suriettiva)] Ogni $[x]_m$ ha una controimmagine, $[x]_n$.
	\end{itemize}
\end{proof}
\begin{proposizione}
...
\begin{proof}
	...
	\end{align*}
	%todo: trovare voglia di mettere in tabular queste due perché stiano sulla stessa riga
	\begin{align*}
	\phi_a:\mathbb{Z}_n&\longrightarrow\mathbb{Z}_a\\
	[x]_n&\longmapsto[x]_a
	\end{align*}
	\begin{align*}
	\phi_b:\mathbb{Z}_n&\longrightarrow\mathbb{Z}_b\\
	...
	\begin{itemize}
		...
		\begin{align*}
		\ker(\phi)&=\left\{[x]_n\in\mathbb{Z}_n \, \mid \, \phi([x]_n)=\left(\phi_a([0]_n),\phi_b([0]_n)\right)\right\}\\
		&=\left\{[x]_n\in\mathbb{Z}_n \, \mid \, \phi_a([0]_n),\phi_b([0]_n)\right\}\\
...
\begin{osservazione}[Gruppo delle unità]
	...
	n &\longmapsto |\mathbb{Z}_n^*|
	\end{align*}
	Notiamo anche che per $a$ e $b$ coprimi ed $n=ab$
	\begin{align*}
	\phi:\mathbb{Z}^*_n&\longrightarrow\mathbb{Z}^*_a\times \mathbb{Z}^*_b\\
	[x]_n&\longmapsto\left([x]_a,[x]_b\right)
	\end{align*}
	...
	\begin{itemize}
		...
		&=\phi([z]_n)\phi([x]_n)=\phi([1]_n)\\
		&=\left([1]_a,[1]_b\right)
		\end{align*}
...
\end{teorema}
\begin{proof}
	La tesi è immediata per teorema di Lagrange. %todo:espandi
\end{proof}
...
Diciamo \textit{congruenze lineari} equazioni del tipo
\begin{equation*}
ax\equiv b\mod(n)
\end{equation*}
Notiamo alcune cose:
...
\begin{controesempio}
	...
	\end{cases}
	\end{equation*}
	%todo: scrivi meglio controesempio
\end{controesempio}
\begin{osservazione}
	...
	\end{cases}
	\end{equation*}
	che sotto le condizioni viste può essere riscritto come %todo: questo non vale solo se gli a_i sono invertibili?
	\begin{equation*}
	\begin{cases}
	x\equiv c_1\mod(n_1)\\
	x\equiv c_2\mod(n_2)
	...
	c_1-c_2=hn_2'-kn_1'
	\end{equation*}
	che per Bezout ha soluzione se e solo se $\MCD(n_1',n_2')|c_1-c_2$.
	%todo: il senso di questa osservazione?
\end{osservazione}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[(iniettiva)]
		\begin{align*}
		\ker(\varphi)&=\left\{x\in\mathbb{Z} \, \mid \, x\equiv0\mod(n),x\equiv0\mod(m)\right\}\\
		%todo: rivedere utilizzo dei \mod, lasciano troppo spazio tra ciò che c'è prima di \mod e la scritta "mod" (in prind: x=0 mod(m))
		&=\left\{x\in\mathbb{Z} \, \mid \, mn|x\right\}=(mn)\mathbb{Z}
		\end{align*}
		\item[(suriettiva)]
		\begin{equation*}
		\sfrac{\mathbb{Z}}{\ker(\varphi)}\simeq\mathbb{Z}_m\times\mathbb{Z}_n\simeq\mathbb{Z}_{mn}
		\end{equation*}
		Allora la funzione è suriettiva. %todo: specifica meglio
	\end{itemize}
	Inoltre se $x$ è soluzione, tutte le soluzioni hanno forma 
	\begin{equation*}
	x+k(mn) \ \ \ \ k\in\mathbb{Z}
	\end{equation*}
	ovvero ho unicità di soluzione in $\mathbb{Z}_{mn}$.
\end{proof}
\begin{proposizione}
	Sia $n=\prod_{i=1}^{n}p_i^{e_i}$, allora 
	\begin{equation*}
...
\begin{proof}
	...
	\begin{align*}
	\varphi:\mathbb{Z}&\longrightarrow\mathbb{Z}_{p_1^{e_1}}\times\ \dots \ \times\mathbb{Z}_{p_n^{e_n}}\\
	x&\longmapsto\left([x]_{p_1^{e_1}},\dots,[x]_{p_n^{e_n}}\right)
	\end{align*}
