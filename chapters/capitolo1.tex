...
\begin{itemize}
	...
	\item gli elementi invertibili di $A[x]$ sono tutti e soli gli elementi invertibili di $A$
\end{itemize}
...
\end{proposizione}
\begin{proof}
	Sicuramente è un anello commutativo con identità, vediamo ce ogni elemento non nullo ha un inverso. Sia $(p)+a$ un elemento di $\sfrac{F[x]}{(p)}$ tale che $a$ non appartenga all'ideale generato da $p$ (questo vuol dire che $p$ non divide $a$). Si noti che $(p)+a=(p)$ se e solo se $a-0\in(p)$, quindi non è il nostro caso. Allora il massimo comune divisore tra $p$ ed $a$ è 1, e per divisione euclidea posso scrivere 
	\begin{equation*}
	1=r(x)a(x)+s(x)p(x)
	\end{equation*}
...
\begin{teorema}[Criterio di Eisenstein]
	Sia $f(x)=a_nx^n+\dots+a_1x+a_0\in\mathbb{Z}[x]$, se esiste un $p$ primo tale che $p$ divide $a_0,a_1,\dots,a_{n-1}$ e $p^2$ non divide $a_0$, allora $f$ è irriducibile in $\mathbb{Q}[x]$. \\ In particolare è irriducibile anche in $\mathbb{Z}[x]$.
\end{teorema}
\begin{esempio}
	...
	\begin{equation*}
	a(x)=x^3+2x+1, {Q}[\ \ b(x)=x^2-2x-3 \ \ \in\mathbb{Q}[x]
	\end{equation*}
