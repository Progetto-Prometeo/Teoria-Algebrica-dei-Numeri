...
\begin{proof}
	...
	Quindi osserviamo che ci basta dimostrare nel caso di due polinomi $f,g \in A[x]$ che 
	...
	Infatti supponendo che valga in quel caso, e non fossero primitivi valrebbe 
	che $f = \operatorname{cont}(f)f'$ per un qualche $f' \in A[x]$ primitivo. Dunque
	\begin{equation*}
	...
	\end{equation*}
	Quindi siano $f,g$ primitivi allora osserviamo che $A/P$ per un qualsiasi ideale primo, induce un omomorfismo 
	$\varphi \colon A[x] \to A/P[x]$. Ma essendo che $f,g$ primitivi e $\varphi$ omomorfismo dev'essere che 
	$\varphi(f),\varphi(g) \not\equiv 0$. Inoltre $A/P[x]$ è un dominio quindi $\varphi(f)\varphi(g) \not\equiv 0$.
	Per proprietà di omomorfismo vale $\varphi(fg) = \varphi(f)\varphi(g)$ segue che 
	$\varphi(fg) \neq 0$ per ogni primo $P$. Ovvero $\operatorname{cont}(fg) = 1$.\\
	
	In particolare se $f$ si fattorizza in prodotto di due polinomi in $K[x]$, dati $g,h \in K[x]$
	\begin{equation*}
		f(x) = g(x)h(x)
	\end{equation*}
	allora sia $g = \operatorname{cont}(g)g'$ con $g' \in A[x]$ e analogamente per $h$. 
	Pertanto $f(x) = \operatorname{cont}(g)\operatorname{cont}(h)g'(x)h'(x)$ ma 
	$\operatorname{cont}(g)\operatorname{cont}(h) = \operatorname{cont}(f) \in A$ e dunque 
	abbiamo trovato una fattorizzazione in $A[x]$ per $f$. 
\end{proof}
\begin{teorema}
...
\end{proof}
\begin{proposizione}
	Sia $A$ un anello di Dedekind, $P$ un suo ideale primo. Detto $K$ il campo delle frazioni di $A$, definisco \footnote{Intuitivamente stiamo definendo l'insieme degli elementi del campo delle frazioni con un elemento di $P$ come \enquote{denominatore}.}
	\begin{equation*}
...
\begin{proof}
	...
	Iniziamo con il vedere che $A \subset P^{-1}$. Sia $0 \neq a\in P$, per il lemma precedente $(a)$ contiene un prodotto di ideali primi $P_1,\dots,P_n$ (scelti con il minor $n$ possibile, 
	questo è ovviamente possibile dato che $n \in \mathbb{N}$). 
