...
\begin{proof}
	...
	\end{equation*}
	Siccome questi due ordini sono coprimi, $\left(g^{pf}\right)(h^{\sfrac{m}{p^e}})$ appartiene a $G$ - perché è un gruppo - ed ha ordine $\frac{n}{pf}p^e=p^{e-f}n>n$; questo contraddice di nuovo la scelta di $n$.
\end{proof}
\begin{teorema}
...
\end{esempio}
\begin{proposizione}
	Per $n$ pari e per $a\in\mathbb{Z}_n^*$, posto $k>2$ e $n=2^k$, si ha 
	\begin{equation*}
...
Il test di Rabin-Miller viene usato in molte funzioni di valutazione della primalità di linguaggi di programmazione, come ad esempio Python. Si tratta a tutti gli effetti di uno dei due test di primalità più usati al mondo, e quindi di un risultato decisamente importante. Sotto l'ipotesi di Riemann diventa inoltre un \emph{criterio} di primalità efficiente; allora condizionato all'ipotesi che sia vera assume un carattere ancora più forte. \\ L'altro test più usato è il \textit{test di Bayle}, che combina il test di Lucas a Fermat fortificato.
\begin{teorema}[Test di Rabin-Miller]
	Sia $n$ numero da valutare, itero $k$ volte il test di Solovay-Strassen. La probabilità che un numero composto superi il test è maggiorata da $\sfrac{1}{4^k}$, ho ottenuto un test più \enquote{potente}.
\end{teorema}
\begin{osservazione}
...
\begin{teorema}
	Dato $F$ campo numerico, il suo anello degli interi $O_F$ è uno $\mathbb{Z}$-modulo.
\end{teorema}
%\begin{proof}
	%todo: quando avrò voglia
%\end{proof}
\begin{definizione}
	Dato $O_F$ anello degli interi di un campo numerico $F$, si dice \textbf{base intera di $O_F$} la sua base come $\mathbb{Z}$-modulo. \\ Analogamente, dati $\alpha_1,\dots,\alpha_n$ in $O_F$ questi si dicono \textbf{base intera} se ogni $\beta$ in $O_F$ si scrive in modo unico come combinazione lineare degli $\alpha_i$ a coefficienti interi.
	\begin{equation*}
...
\begin{proposizione}[Proprietà delle basi intere] \
	\begin{enumerate}
		\item Sia $I$ ideale non-nullo di $O_F$, allora esiste un $c$ non-nullo appartenente ad $I\cap\mathbb{Z}$.
		\item Sia $I$ un ideale non-nullo di $O_F$, allora esistono degli $\alpha_1,\dots,\alpha_n\in I$ con $n$ grado di $F$ estensione di $\mathbb{Q}$ e 
		\begin{equation*}
		\disc(\alpha_1,\dots,\alpha_n)\in I
		\end{equation*}
		\item Sia $F$ estensione di $\mathbb{Q}$ di grado $n$, $I$ un ideale non-nullo in $O_F$. Allora esiste una base intera di $I$. Allora $I$ è uno $\mathbb{Z}$-modulo libero di rango $n$.
		\item Sia $I$ ideale non-nullo di $O_F$ con due basi intere $\alpha_1,\dots,\alpha_n$ e $\beta_1,\dots,\beta_n$. Allora il loro discriminante è uguale.
	\end{enumerate}
\end{proposizione}
\begin{proof}\
	\begin{enumerate}
		...
		\end{equation*}
		allora è evidente che $\alpha^m+\dots+b_1\alpha+b_0=0$, in particolare ottengo che $b_0\in I$, ed è non-nullo perché il polinomio è irriducibile.
		\item Prendo $\omega_1,\dots,\omega_n$ base del $\mathbb{Q}$-spazio vettoriale $F$, abbiamo visto che il suo discriminante è non-nullo. Inoltre sappiamo che esiste un $c$ non-nullo appartenente ad $I\cap\mathbb{Z}$. Poniamo $\alpha_i=c\omega_i\in I$, siccome 
		\begin{equation*}
		...
		\begin{equation*}
		S = \left\{\left|\disc(\alpha_1,\dots,\alpha_n) \ \mid \ \text{il discriminante è non-nullo ed $\alpha_i\in I$}\right| \right\}
		\end{equation*}
		...
		%todo: forma esplicita delle matrici? quando avrò voglia
		\begin{equation*}
		...
		\item Per esercizio.
		%todo: farla, suppongo basti usare le proprietà del determinante, non ho voglia di scriverla.
	\end{enumerate}
\end{proof}
\begin{corollario}
	$=O_F$ è un anello Noetheriano.
\end{corollario}
\begin{proof}
...
\end{proof}
\begin{definizione}
	Definisco \textbf{discriminante di $I$ ideale di $O_F$} come il discriminante di una base intera di $I$. Abbiamo visto che non dipende dalla scelta della base intera.
\end{definizione}
...
Sia $F$ un campo quadratico, ovvero una estensione finita di grado 2 su $\mathbb{Q}$. Un qualunque elemento $\alpha$ preso in $F\setminus\mathbb{Q}$ è \textit{primitivo}, ovvero 
\begin{equation*}
F=\mathbb{Q}(\alpha)
\end{equation*}
...
\begin{osservazione}
	...
	\begin{enumerate}
		...
		\item $\alpha\in O_F$ se e solo se $\N(\alpha),\tr(\alpha)\in\mathbb{Z}$, l'implicazione da sinistra a destra vale in generale, l'altra in generale è falsa
	\end{enumerate}
\end{osservazione}
\begin{teorema}[Caratterizzazione degli anelli degli interi]
	Sia $F=\mathbb{Q}(\sqrt{d})$ con $d$ intero libero da quadrati, allora 
	\begin{equation*}
	O_F=\left\{a+b\sqrt{d}\ \mid \ a,b\in\mathbb{Z}\right\}=\mathbb{Z}(\sqrt{d}) \ \ \ \text{se $d\equiv2,3\Mod4$}
	\end{equation*}
	\begin{equation*}
	O_F=\left\{\frac{a+b\sqrt{d}}{2}\ \mid \ a,b\in\mathbb{Z}, a\equiv b\Mod2\right\} \ \ \ \text{se $d\equiv1\Mod4$}
	\end{equation*}
\end{teorema}
\begin{proof}
	Sia $\alpha\in O_F$, possiamo scriverlo come 
	\begin{equation*}
	\alpha=\frac{a+b\sqrt{d}}{2}
	\end{equation*}
	con $a,b,c\in\mathbb{Z}$ e $\MCD(a,b,c)=1$, assumo $c>0$ senza perdita di generalità. Siccome $\alpha\in O_F$, per quanto visto prima 
	\begin{enumerate}
...
\end{proof}
\begin{corollario}
	Se $d\equiv2,3\Mod4$, ho come base intera di $O_F$ $1$ e $\sqrt{d}$. In particolare 
	\begin{equation*}
	O_F=\mathbb{Z}[\sqrt{d}]
	\end{equation*}
	Se invece $d\equiv1\Mod4$, ho come base intera di $O_F$ $1$ e $\frac{1+\sqrt{d}}{2}$. In particolare
	\begin{equation*}
	O_F=\mathbb{Z}[\frac{a+b\sqrt{d}}{2}]
		\end{equation*}
\end{corollario}
\begin{proof}
...
\end{proof}
\begin{proposizione}
	Lo sviluppo in frazione continua di $\alpha_0$ è continuo se e solo se $\alpha_0$ è una irrazionalità quadratica, ovvero soluzione irrazionale di una equazione di secondo grado. \footnote{Ad esempio per $\alpha_0=\sqrt{2}$ abbiamo una espressione periodica tramite frazione continua; questo è fondamentale per poter \enquote{possedere} veramente il numero, cioè per poterlo ricostruire partendo da informazioni elementari, quindi nel nostro caso non dicendo \enquote{è soluzione positiva di $x^2-2$} e nemmeno \enquote{è radice di 2}. Per le irrazionalità cubiche e di grado superiore il problema è ancora aperto, Jacobi ha provato a risolverlo ma soddisfacendo solo una implicazione.}
\end{proposizione}
\begin{esempio}
	Lavoriamo su $\sqrt{2}$.
	\begin{align*}
