...
Il test di Rabin-Miller viene usato in molte funzioni di valutazione della primalità di linguaggi di programmazione \enquote{matematici}, come ad esempio Python. Si tratta a tutti gli effetti di uno dei due test di primalità più usati al mondo, e quindi di un risultato decisamente importante. Sotto l'ipotesi di Riemann diventa inoltre un criterio di primalità efficiente; in altre parole, se condizionato all'ipotesi che sia vera assume un carattere ancora più forte: diventa un test \textit{deterministico}. \\ L'altro test più usato è il \textit{test di Bayle}, che combina il test di Lucas a Fermat fortificato, ma non ce ne occuperemo in questo corso.
...
\begin{proof}
	...
	%todo: spiega il logaritmo intero
\end{proof}
\begin{lemma}
...
\begin{proof}
	...
	&=2^{s-1}\MCD(m,p-1)
	\end{align*}
	Questo perché se $p$ divide $n$ e $m$ divide $n-1$ abbiamo che $\MCD(p,m)=1$ ed il massimo comune divisore tra $2^{s-1}m$ e $p^{\alpha_p-1}$ è 1. Nell'ultima uguaglianza ho usato la definizione di $s$ come massima potenza di 2 che divide tutti i $p-1$. Allora 
	\begin{equation*}
...
\begin{proof}
	...
	\end{equation*}
	Le frazioni sono due numeri interi perché $p-1$ diviso $2^s$ è un dispari, e viene diviso per un suo divisore. Perché la disuguaglianza continui a valere allora deve necessariamente essere
	\begin{equation*}
...
\end{lemma}
\begin{proof}
	%todo: scrivi in forma esplicita p ed n
...
\begin{proof}
	...
	\begin{align*}
	\sigma_i\left(c_1x_1+\dots+c_nx_n\right)
...
\end{teorema}
\begin{proof}
	Devo provare prima di tutto che $\mathcal{O}_F$ sia un gruppo commutativo rispetto alla somma, con elemento neutro 0. Ma questo è abbastanza ovvio, intuitivamente posso costruire il polinomio minimo (monico) con la procedura vista in precedenza. \\
	Il resto della definizione è banale per le proprietà di $\mathbb{Z}$.
\end{proof}
\begin{definizione}
...
\begin{proof}\
	\begin{enumerate}
		...
		\item Prendo $\omega_1,\dots,\omega_n$ base dello $\mathbb{Q}$-spazio vettoriale $F$, abbiamo visto che il suo discriminante è non-nullo. Scelgo questi $\omega_i$ perché si trovino in $\mathcal{O}_F$, ad esempio detto $F=\mathbb{Q}(k)$ potrei prendere $1,\dots,k^{n-1}$. Inoltre sappiamo che esiste un $c$ non-nullo appartenente ad $I\cap\mathbb{Z}$. Poniamo $\alpha_i=c\omega_i\in I$, siccome 
		\begin{equation*}
...
\begin{corollario}
	...
	\begin{equation*}
	\mathcal{O}_F=\mathbb{Z}[\frac{a+b\sqrt{d}}{2}]
		\end{equation*}
\end{corollario}
\begin{proof}
...
\begin{esempio}
	...
	\begin{enumerate}
		...
		\begin{align*}
		\left(x-\left(\sqrt{-2}+\sqrt{-5}\right)\right)&\left(x-\left(-\sqrt{-2}+\sqrt{-5}\right)\right)\dots\\
...
\subsection{Gli anelli di Dedekind}
\begin{definizione}[Anello di Dedekind]
	Sia $A$ un anello, viene detto \textbf{anello di Dedekind} de
	\begin{enumerate}
		\item è integralmente chiuso
		\item è Noetheriano
		\item ogni ideale primo non nullo è massimale
	\end{enumerate}
\end{definizione}
\begin{esempio}
	$\mathbb{Z}$ è un anello di Dedekind. In seguito lavoreremo per mostrare che $\mathcal{O}_F$ è un anello di Dedekind per $F$ campo numerico.
\end{esempio}
\begin{proposizione}\
	\begin{enumerate}
...
\begin{proof}\
	\begin{enumerate}
		\item Sia $c\in C$; siccome $C$ è intero su $B$, $B[c]$ è un $B$-modulo finitamente generato. Siccome $B$ è intero su $A$, $B[c]$ è finitamente generato come $A$-modulo. In conclusione $A[c]\subset B[c]\subset C$, quindi per caratterizzazioni viste in precedenza $c$ è intero su $A$.
...
\end{teorema}
\begin{proof}
	I punto della definizione sono dimostrati dalle proposizioni.
\end{proof}
