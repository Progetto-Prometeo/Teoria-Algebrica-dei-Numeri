...
\end{definizione}
\begin{osservazione}
	Prima di tutto vediamo che il massimo comune divisore esiste sempre, difatti appartiene ad un insieme con estremo inferiore $1$ ed estremo superiore il massimo dei moduli di $a$ e $b$.
...
\end{teorema}
\begin{proof}
	Dimostriamo che $q$ ed $r$ esistono. \\ Supponiamo $a\geq 0$ e $b>0$ senza perdita di generalità; se $a=0$ basta prendere $q=r=0$. Possiamo quindi fissare $b$ e procedere per induzione su $a$. L'ipotesi induttiva è che tali $r$ e $q$ esistano per ogni coppia $(n,b)$ con $n<a$; voglio mostrare che esistono anche per la coppia $(a,b)$.
	\begin{itemize}
		...
		\begin{equation*}
		c = q'b + r' \ \text{e} \ 0 \leq r' \leq |b|
		\end{equation*}
		e chiaramente ottengo subito
		\begin{equation*}
		a = b+c = (q'+1)b + r' \ \text{e} \ 0 \leq r' \leq |b|
		\end{equation*}
	\end{itemize}
	Dimostriamo ora l'unicità. \\
	Siano $q$ e $q'$, $r$ ed $r'$ tali che 
	\begin{equation*}
	a = qb + r= q'b + r'\ \text{e} \ 0 \leq r,r' \leq |b|
	\end{equation*}
...
\begin{osservazione}[Algoritmo di Euclide] Possiamo costruire un algoritmo per il calcolo del massimo comune divisore di due $a$ e $b$ generici in $\mathbb{Z}$. Ma come? La dimostrazione che abbiamo visto non è costruttiva. \\ \\ Facciamo alcune osservazioni.
	\begin{enumerate}
		...
		\begin{equation*}
			A = \left\{ d \in \mathbb{Z} : d \mid a \land d \mid b \right\}
		\end{equation*}
		\begin{equation*}
			B = \left\{ d \in \mathbb{Z} : d \mid b \land d \mid r \right\}
		\end{equation*}
...
\begin{proof} Dimostriamo che esiste. \\ Suppongo senza perdita di generalità che $n>0$, se $n=2$ ho già la fattorizzazione
	...
	\end{equation*}
	Posso osservare che $p_1$ divide $n$, ed essendo primo si ha che $p_1$ divide un $q_i$ per un qualche indice $i$. Ma siccome $q_i$ è irriducibile deve necessariamente essere $p_1=q_i$. Allora ottengo una seconda fattorizzazione rimuovendoli, e ripetendo il ragionamento $s$ volte otterrei (a meno di riordinamento)
	\begin{equation*}
	1 = q_{s+1}\cdot \ \dots \ q_t
	\end{equation*}
	che è un assurdo siccome 1 non ha fattori irriducibili: allora deve essere $s=t$. \\ Abbiamo già visto la necessità di $p_j = q_i$ per certi indici $i$ e $j$ opportuni, questo chiude la dimostrazione.
\end{proof}
...
\begin{proof}
	...
	\begin{equation*}
	\left\{\text{divisori di $nm$}\right\}\leftrightarrow\left\{\text{divisori di $n$}\right\}\times\left\{\text{divisori di $m$}\right\}
	\end{equation*}
\end{proof}
...
\epigraph{\textit{\enquote{Dio ha creato gli interi, tutto il resto è opera dell'uomo.}}}{Leopold Kronecker}
\label{lezione3}
...
\begin{teorema}[Esistenza della fattorizzazione]
	Se $A$ è principalmente noetheriano allora posso fattorizzare ogni suo elemento non-nullo e diverso da un'unità come prodotto di elementi irriducibili.
\end{teorema}
\begin{proof}
...
\epigraph{\textit{\enquote{Perché i numeri sono stupendi? Sarebbe come chiedere perché la Nona Sinfonia di Beethoven è stupenda. Se non ne vedi il motivo, nessuno può spiegartelo. Io so che i numeri sono stupendi. Se non lo sono loro, nulla lo è.}}}{Paul Erd\"os}
...
\begin{proof}
	...
	\begin{itemize}
		...
		\item[(iniettiva)] Vediamo che $\ker \phi = \left\{0\right\}$: se $x \in \ker \phi$ allora $\phi(x) = g^x = 1$. Siccome $g$ è un generatore, $g^x$ non può essere l'identità per valori più piccoli di $n$, quindi $n \mid x$. Quindi $x \equiv 0 \Mod n$, ovvero $x \equiv 0 \in \mathbb{Z}_n$. Si noti l'utilizzo della struttura ciclica di $G$.
...
\end{esercizio}
\begin{osservazione}
	Il criterio di Eulero permette di caratterizzare i primi $p$ per cui $-1$ è un residuo quadratico modulo $p$. \\ Osserviamo che tutti i primi (dispari) sono congrui a 3 o a 1 modulo 4.
	\begin{itemize}
		\item Se $p\equiv 1 \Mod 4$, allora ${(-1)}^{\frac{p-1}{2}} = 1$, quindi per il criterio di Eulero $\leg{-1}{p}=1$, cioè $-1 \in Q_p$
		\item Analogamente, se $p\equiv 3 \Mod 4$, allora ${(-1)}^{\frac{p-1}{2}} = -1$, quindi per il criterio di Eulero $\leg{-1}{p}=-1$, cioè $-1 \notin Q_p$
	\end{itemize}
\end{osservazione}
...
\end{proof}
\begin{osservazione}
	Il test di Wilson è deterministico, tuttavia la sua applicazione non è facile; infatti la presenza del fattoriale lo rende estremamente difficile da calcolare. Vedremo altri test, non deterministici più efficienti. 
\end{osservazione}
\begin{lemma}
...
\begin{proof}
	...
	Sia allora $t=2$: ho due primi $p_1$ e $p_2$, che posso supporre maggiori o uguali di 3. Il caso $p_i=2$ infatti è banale e poco interessante. Vale per la supposizione assurda che 
	\begin{equation*}
	...
	\end{equation*}
	Le frazioni sono due numeri interi perché $p-1$ diviso $2^s$ è un dispari, e viene diviso per un dispari. Affinché continui a valere la disuguaglianza deve necessariamente essere
	\begin{equation*}
		\begin{array}{l}
...
\end{proof}
\begin{proof}
	Un'altra dimostrazione dovuta a Dedekind che usa la fattorizzazione unica di $\mathbb{Z}[i]$.\\
	
	Sia $p \equiv 1 \mod 4$ allora sappiamo che esiste $m$ intero tale che $p \mid m^2  + 1$. 
	Osserviamo che $m^2 + 1 = (m-i)(m+i)$ in $\mathbb{Z}[i]$, ma $p \nmid m-i$ e $p \nmid m+i$ poiché 
	$p \nmid i$. Per cui concludiamo che $p$ non è primo in $\mathbb{Z}[i]$. 
	Vale che $N(p) = p^2$ e dunque, dalla fattorizzazione della norma, 
	otteniamo che possono essere al massimo due fattori $a,b$ tali che $N(a) = N(b) = p$ e che $p = ab$.
	Osserviamo che $N(b) = p$ allora vuol dire che $b = x+yi$ con $x^2 + y^2 = p$, da cui la tesi.  
\end{proof}
...
\end{proof}
\begin{proof}[Un'altra dimostrazione del secondo complemento]
	Osserviamo che basta applicare il Lemma di Gauss, ricordando che $S$ è quell'insieme tale che
	\begin{equation*}
		2k \in S \iff 2k > \frac{p}{2} \land k < \frac{p-1}{2} \iff k > \left\lfloor \frac{p}{4}\right\rfloor \land k < \frac{p-1}{2}	
	\end{equation*}
 	È dunque evidente che $|S| = \frac{p-1}{2} - \left\lfloor\frac{p}{4} \right\rfloor$.
	Ora verifichiamo i risultati modulo $8$, calcoliamo $|S|$ nei vari casi.
	\begin{enumerate}
		\item[$(p \equiv 1 \mod 8)$] 
		\begin{equation*}
			|S| \equiv \frac{p-1}{2} - \left\lfloor\frac{p}{4}\right\rfloor  \equiv 0 - 0 \equiv 0 \mod 2
		\end{equation*}
		\item[$(p \equiv 3 \mod 8)$]
		\begin{equation*}
			|S| \equiv \frac{p-1}{2} - \left\lfloor\frac{p}{4} \right\rfloor \equiv 1 - 0 \equiv 1 \mod 2	
		\end{equation*}
		\item[$(p \equiv 5 \mod 8)$]
		\begin{equation*}
			|S| \equiv \frac{p-1}{2} - \left\lfloor\frac{p}{4}\right\rfloor  \equiv 0 - 1 \equiv 1 \mod 2
		\end{equation*}
		\item[$(p \equiv 7 \mod 8)$]
		\begin{equation*}
			|S| \equiv \frac{p-1}{2} - \left\lfloor\frac{p}{4}\right\rfloor  \equiv 1 - 1 \equiv 0 \mod 2
		\end{equation*}
	\end{enumerate}
	Da cui otteniamo la tesi. Notiamo che sono tutti i casi possibili per $p$ primo: ovviamente un primo non può essere congruo a 2, a 4 e neppure a 0 in modulo 8. Se così fosse non sarebbe un primo.
\end{proof}
\begin{proof}
	\label{lezione13}
...
\epigraph{\textit{\enquote{Tramite l'algebra arriviamo facilmente a delle equazioni, ma dobbiamo sempre passare attraverso costrutti e dimostrazioni eleganti.}}}{Isaac Newton}
...
\end{teorema}
\begin{definizione}
    Per $p$ polinomio a coefficienti in $\mathbb{Z}$, definiamo $\operatorname{cont}(p)$ il \textbf{contenuto di $p$} come il massimo comune divisore dei suoi coefficienti. \\ \\ Siccome possiamo definire la \textbf{parte primitiva di $p$} $\operatorname{prim}(p)$ come il quoziente tra $p$ ed il suo contenuto, risulta evidente che
    \begin{equation*}
        p = \operatorname{prim}(p) \cdot \operatorname{cont}(p)
    \end{equation*}
    e si può dimostrare che questa fattorizzazione è unica.
\end{definizione}
...
\end{teorema}
\begin{proof}
	Possiamo dimostrarlo nel caso più generale in cui $A$ è un UFD e $K$ campo delle frazioni di $A$. 
	Quindi osserviamo che ci basta dimostrare nel caso di due polinomi $f,g \in A[x]$ con 
	$\operatorname{cont}(fg) = \operatorname{cont}(f)\operatorname{cont}(g)$. 
	Vediamo che possiamo ridurci al caso in cui $f,g$ primitivi, ovvero $\operatorname{cont}(f),\operatorname{cont}(g) = 1$.
	Infatti se ho la tesi per polinomi primitivi e prendo $f$, $g$ non primitivi vale che $f = \operatorname{cont}(f)f'$ per un qualche $f' \in A[x]$ primitivo. Dunque
	\begin{equation*}
		\operatorname{cont}(fg) = \operatorname{cont}(\operatorname{cont}(f)f'\operatorname{cont}(g)g') = \operatorname{cont}(f)\operatorname{cont}(g)
	\end{equation*}
	Quindi siano $f,g$ primitivi, osserviamo che $A/P$ per un qualsiasi ideale primo induce un omomorfismo 
	$\varphi \colon A[x] \to A/P[x]$. Ma dato che $f,g$ sono primitivi e $\varphi$ è un omomorfismo dev'essere anche
	$\varphi(f),\varphi(g) \not\equiv 0$. Inoltre $A/P[x]$ è un dominio, quindi $\varphi(f)\varphi(g) \not\equiv 0$.
	Per proprietà dell'omomorfismo vale $\varphi(fg) = \varphi(f)\varphi(g)$, e segue che 
	$\varphi(fg) \neq 0$ per ogni primo $P$, ovvero $\operatorname{cont}(fg) = 1$.\\
	In particolare se $f$ si fattorizza nel prodotto di due polinomi in $K[x]$
	\begin{equation*}
		f(x) = g(x)h(x)
	\end{equation*}
	sia $g = \operatorname{cont}(g)g'$ con $g' \in A[x]$, ed analogamente per $h$; in questo caso otteniamo 
	\begin{equation*}
	    f(x) = \operatorname{cont}(g)\operatorname{cont}(h)g'(x)h'(x)
	\end{equation*}
	ma sappiamo anche che
	\begin{equation*}
	    \operatorname{cont}(g)\operatorname{cont}(h) = \operatorname{cont}(f) \in A
	\end{equation*}
	e dunque 
	abbiamo trovato una fattorizzazione di $f$ in $A[x]$.
\end{proof}
\begin{teorema}
...
\begin{esercizio}
	Un analogo procedimento si può usare per risolvere l'equazione del tipo $x^3 = y^2 + 2$. Vedere perché non 
	si può estendere lo stesso procedimento per $x^3 = y^2 + 3$.
\end{esercizio}
...
\epigraph{\textit{\enquote{Proprio come i numeri razionali negativi e frazionari sono formati tramite nuova creazione, e come le leggi delle operazioni con questi numeri possono e devono essere ricondotte alle leggi di operare con i numeri positivo, così dobbiamo sforzarci di definire i numeri irrazionali per mezzo dei soli numeri razionali. Resta solo la domanda: come farlo?}}}{Richard Dedekind}
...
\begin{proof}
	...
	\end{equation*}
	con $b_j\neq\frac{\theta-\alpha_i}{\lambda}$, ovvero $\MCD(h,g)=x-\beta$.
	\footnote{Prendiamo il polinomio minimo di 
		$\beta$ in $\mathbb{Q}(\theta)[x]$, sia esso $k(x)$, 
		allora $k \mid h$ e $k\mid g$ a questo punto è facile 
		osservare che siccome l'unica in radice in comune tra 
		$g,h$ è $\beta$ dev'essere che $k(x) = x-\beta$.} 
	Questo equivale a dire che $\beta\in\mathbb{Q}(\theta)$. 
	In modo analogo posso ottenere $\alpha\in\mathbb{Q}(\theta)$.
...
\begin{proof}\
	\begin{enumerate}
		...
		\end{equation*}
		\begin{equation*}
		\beta=(b_1-k)\gamma_1+b_2\gamma_2+\dots+b_n\gamma_n
		\end{equation*}
...
\section{Fattorizzazione unica in ideali}
...
Tuttavia ciò che possiamo fare è garantire o meno l'esistenza e l'unicità della \textit{fattorizzazione in ideali primi} di un certo ideale $J$ di un anello $A$.
\begin{esempio}
	...
	\end{equation*}
\end{esempio}
\begin{teorema}[Teorema di Dedekind, forma base]
...
\begin{definizione}
    Un anello $A$ viene detto \textbf{integralmente chiuso rispetto ad $R$ suo sottoanello} se ogni $\rho\in R$ zero di un polinomio monico a coefficienti in $A$ è contenuto in $A$.
\end{definizione}
\begin{definizione}[Anello di Dedekind]
	Sia $A$ un anello, viene detto \textbf{anello di Dedekind} se
	\begin{enumerate}
		\item è integralmente chiuso rispetto al suo campo delle frazioni
		\item è Noetheriano
		\item ogni suo ideale primo non nullo è massimale
	\end{enumerate}
\end{definizione}
...
\end{esempio}
\begin{lemma}
    Un dominio di integrità finito è un campo.
\end{lemma}
\begin{proof}
	Una possibile dimostrazione: sia $|D| = n$ per $D$ dominio finito. Supponiamo per assurdo esista $0 \neq d \in D$ tale che per ogni $a \in D$ valga $da \neq 1$. 
	Allora $|dD| \le |D|-1$, per cui esistono $0 \neq \beta, \gamma \in D$ e $\beta \neq \gamma$ tali che $d\gamma = d\beta \in D$. 
	Quindi $d(\gamma-\beta) = 0$, e poiché è un dominio di integrità dev'essere che $\gamma = \beta$, contro l'ipotesi $\beta \neq \gamma$, da cui l'assurdo; dev'essere che anche $d$ ammette un inverso.
\end{proof}
\begin{proposizione}[Caratterizzazioni degli anelli degli interi]\
	\begin{enumerate}
...
\begin{proof}\
	\begin{enumerate}
		...
		\begin{enumerate}
			...
			\item Se $I$ è primo, $\mathcal{O}_F/I$ è un dominio di integrità. Siccome ogni dominio di integrità finito è un campo, $\mathcal{O}_F/I$ è un campo. Allora $I$ è massimale.
		\end{enumerate}
	\end{enumerate}
\end{proof}
...
\begin{proof}
	...
	\end{equation*}
	e sia $J$ l'elemento più grande rispetto all'inclusione - qui posso usare il fatto che l'anello sia di Dedekind, e quindi noetheriano. Per teorema (che non vedremo), esiste un $P$ tale che $J$ è contenuto in $P$, inoltre
	\begin{equation*}
	J\subset JP^{-1}, \ JP^{-1}\neq A
	\end{equation*}
	Ne segue che 
	...
	\end{equation*}
    che è un assurdo sulla richiesta di $J\in\mathcal{M}$. \\ \\
	Per l'unicità invece supponiamo che 
	\begin{equation*}
